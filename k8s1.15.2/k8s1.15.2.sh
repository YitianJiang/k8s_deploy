...
scp base.config hwclock_ntp.sh node_install_k8s.sh ssh_trust_init.exp ssh_trust_add.exp root@$host:/root && \
scp /etc/hosts root@$host:/etc/hosts && ssh root@$host "hostnamectl set-hostname $hostname$num" && \
ssh root@$host && sh /root/hwclock_ntp.sh && ssh root@$host && sh /root/node_install_k8s.sh && \
ssh root@$host "rm -rf base.config node_install_k8s.sh ssh_trust_init.exp ssh_trust_add.exp hwclock_ntp.sh"
fi
echo $host"服务器安装完毕！！！ "
done
}
